...
export default function MockTestDetailPage() {
  ...
  const handleStartTest = async () => {
    ...
    try {
      ...
      setStartError(
        err instanceof Error ? err.message : "Could not start mock test. Please try again."
      );
    } finally {
      setStarting(false);
    }
  ...
  if (!slug) {
    return (
      <PageLayout title="Mock Test" icon="timer">
        <p className="text-slate-600 dark:text-slate-400">Invalid test.</p>
        <Link href="/moctest" className="mt-4 text-[var(--primary)] hover:underline">
          Back to Mock Tests
        </Link>
      </PageLayout>
    );
  ...
  if (loading) {
    return (
      <PageLayout title="Loading…" icon="timer">
  ...
  if (error || !test) {
    return (
      <PageLayout title="Error" icon="timer">
        <p className="text-red-600 dark:text-red-400">{error || "Mock test not found."}</p>
        <Link href="/moctest" className="mt-4 text-[var(--primary)] hover:underline">
          Back to Mock Tests
        </Link>
      </PageLayout>

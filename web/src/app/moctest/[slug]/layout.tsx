...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  try {
    const path = isUuid(slug)
      ? `/mock-tests/published/${slug}`
      : `/mock-tests/published/slug/${encodeURIComponent(slug)}`;
    ...
    return {
      ...
      description: test.description ?? "Mock test.",
    };
  } catch {
...
export default function MockTestSlugLayout({ children }: Props) {
  return <>{children}</>;
}

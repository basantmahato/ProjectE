...
export class NotificationsService {
  ...
  private getVisibilityCondition(
    userId?: string | null,
  ...
  async findAll(
    ...
    const visibilityCondition = this.getVisibilityCondition(
      userId,
      options.viewerIsAdmin ?? false,
    );

    const countQuery = db
      .select({ count: sql<number>`count(*)::int` })
      .from(notifications);
    const [countResult] = visibilityCondition
      ? await countQuery.where(visibilityCondition)
      : await countQuery;
    ...
    const rowsQuery = db
      ...
      .limit(limitNum)
      .offset(offset);
    const rows = visibilityCondition
      ? await rowsQuery.where(visibilityCondition)
      : await rowsQuery;
    ...
    const data = rows.map((row) => {
      const createdAt =
        row.createdAt ?? (row as unknown as { created_at?: Date }).created_at;
      return {
        id: row.id,
        title: row.title,
        body: row.body,
  ...
  async markAsRead(
    ...
    await this.findOne(notificationId, userId, options);
    await db
      .insert(userNotificationRead)
      .values({ userId, notificationId })
      .onConflictDoNothing({
        target: [
          userNotificationRead.userId,
          userNotificationRead.notificationId,
        ],
      });
  }
  ...
  async findOne(
    ...
    const [row] = await db
      ...
      .where(
        visibilityCondition
          ? and(eq(notifications.id, id), visibilityCondition)
  ...
  async createAndSendToUser(
    ...
    const [created] = await db
      ...
      .values({ userId, title, body: body ?? null, type })
  ...
  private async sendWebPushToSubscriptions(
    ...
    for (const sub of subscriptions) {
      try {
        ...
        this.logger.warn(
          `Web push failed for endpoint: ${sub.endpoint?.slice(0, 50)}`,
        );
      }
    }
    return successCount;
